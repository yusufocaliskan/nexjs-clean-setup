...
const CoolButton = ({disabled, type, selected, label, onClick, fullSize, style}) => {
  return (
    <div className="cool-button">
      ...
      {type === 'Main' && !fullSize && (
        <div className="btn-main" onClick={onClick} style={({cursor: 'pointer'}, {...style})}>
          {label}
        </div>
      )}
      {type === 'Small' && (
      ...
      {type === 'Main&Small&Plain' && (
        <div onClick={onClick} className="btn-small-padding-main" style={[{cursor: 'pointer'}, ...style]}>
          {label}
        </div>
      )}
    </div>
